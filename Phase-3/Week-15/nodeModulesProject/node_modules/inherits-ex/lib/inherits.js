...
function _inherits(ctor, superCtor, staticInherit) {
  let v = (0, _getSuperCtor.default)(ctor);
  ...
  let result = false;
  const isInherited = (0, _isInheritedFrom.isInheritedFrom)(ctor, superCtor);
  if (!isInherited && !(0, _isInheritedFrom.isInheritedFrom)(superCtor, ctor)) {
    ...
    */
    ...
    if (!ctor.hasOwnProperty('__super__')) {
      (0, _defineProperty.defineProperty)(ctor, '__super__', superCtor.prototype);
    }
  }
  return result;
}
 ...
 */
function inherits(ctor, superCtors, staticInherit) {
  if (!isArray(superCtors)) {
    return _inherits(ctor, superCtors, staticInherit);
  }
  for (let i = superCtors.length - 1; i >= 0; i--) {
    if (!_inherits(ctor, superCtors[i], staticInherit)) {
      return false;
    }
  }
  return true;
}
var _default = inherits;
