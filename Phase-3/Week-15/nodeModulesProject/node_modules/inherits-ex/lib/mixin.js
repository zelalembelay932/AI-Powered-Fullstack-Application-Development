...
exports.filterOpts = exports.default = void 0;
exports.mixin = mixin;
...
function _cloneCtor(dest, src, filter) {
  const filterFn = function (name, value) {
    if (['length', 'name', 'arguments', 'caller', 'prototype'].includes(name)) {
      return;
    }
    if (value !== undefined) {
  ...
  (0, _clone2._clone)(dest, src, filterFn);
}
...
function _clonePrototype(dest, src, filter) {
  ...
  (0, _clone2._clone)(dp, sp, filterFn);
  /*
...
function mixin(ctor, superCtor, options) {
  const v = (0, _getSuperCtor.default)(ctor); // original superCtor
  let result = false;
  // Check if the two classes are already related in some way to avoid circular or duplicate inheritance
  if (!(0, _isMixinedFrom.isMixinedFrom)(ctor, superCtor) && !(0, _isInheritedFrom.isInheritedFrom)(ctor, superCtor) && !(0, _isInheritedFrom.isInheritedFrom)(superCtor, ctor)) {
    ...
    if (!mixinCtor) {
      ...
      (0, _defineProperty.defineProperty)(ctor, 'mixinCtor_', mixinCtor);
      if (v && v !== Object) {
        (0, _inheritsDirectly.inheritsDirectly)(mixinCtor, v);
      }
    ...
    if (!mixinCtors) {
      mixinCtors = [];
      (0, _defineProperty.defineProperty)(ctor, 'mixinCtors_', mixinCtors);
    }
    mixinCtors.push(superCtor); // quickly check in isMixinedFrom.
    const filterFn = _getFilterFunc(options && options.filter);
    (0, _clone2.cloneCtor)(mixinCtor, superCtor, filterFn);
    (0, _clone2.clonePrototype)(mixinCtor, superCtor, filterFn);
    if (v !== mixinCtor) {
      (0, _inheritsDirectly.inheritsDirectly)(ctor, mixinCtor);
    }
    result = true;
  }
  return result;
...
function mixins(ctor, superCtors, options) {
  ...
  for (let i = 0; i < superCtors.length; i++) {
    ...
    if (!mixin(ctor, superCtor, options)) {
      return false;
    }
  }
  return true;
